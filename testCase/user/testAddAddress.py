...
class AddAddress(unittest.TestCase):
    ...
    def checkResult(self):
        ...
        if self.result == '0':
            ...
            self.assertEqual(utils.get_value(self.info, 'address', 'sex'), self.sex)
            self.assertEqual(utils.get_value(self.info, 'address', 'fname'), self.fname)
            self.assertEqual(utils.get_value(self.info, 'address', 'lname'), self.lname)
            self.assertEqual(utils.get_value(self.info, 'address', 'tel'), self.tel)
            self.assertEqual(utils.get_value(self.info, 'address', 'address1'), self.address1)
            self.assertEqual(utils.get_value(self.info, 'address', 'city'), self.city)
            self.assertEqual(utils.get_value(self.info, 'address', 'state'), self.state)
            self.assertEqual(utils.get_value(self.info, 'address', 'postcode'), self.postcode)
            self.assertEqual(utils.get_value(self.info, 'address', 'countryId'), self.country_id)
